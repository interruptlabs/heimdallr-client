...
class LazyFileSection(object):
    ...
    "Lazy" because it waits for data to be needed to decompress it. Approximately 75% performance increase
    versus doing it all at once for getting MD5 hash
    """
    def __init__(self, wbits, in_fh):
        import zlib
        from io import BytesIO

        self.zlib = zlib.decompressobj(wbits)
        self.in_fh = in_fh
        self.out_fh =  BytesIO(None)
        self.out_size = 0
    ...
    def read(self, size):
        ...
        start = self.out_fh.tell()
        min_size = start + size
           
        while self.out_size < min_size:
            self.out_fh.seek(0, 2) # Seek EOF
            # Write 16k of data
            data = self.zlib.decompress(self.in_fh.read(self.chunk_size))
            write_len = self.out_fh.write(data)
            self.out_size += write_len
            if write_len == 0:
                break
        self.out_fh.seek(start, )
        return self.out_fh.read(size)
    ...
    def seek(self, offset, *args):
        ...
        self.out_fh.seek(min_size, 0)
    
    def tell(self):
        return self.out_fh.tell()
...
# id2 files start with 'IDAS\x1d\xa5\x55\x55'
    ...
    def getsection_lazy(self, cls):
        return cls(self, self.getpart(cls.INDEX))

    def getpart_lazy(self, ix):
        comp, ofs, size, checksum = self.getsectioninfo(ix)
        ...
        if comp == 2:
            ...
            # very old databases used a different compression scheme:
            wbits = -15 if self.magic == 'IDA0' else 15
            return LazyFileSection(wbits, fh)
        elif comp == 0:
            pass
        else:
            raise Exception("unsupported section encoding: %02x" % comp)
    ...
    def get_hash_fast(self) -> bytes:
        cls = ID0File
        fh = self.getpart_lazy(cls.INDEX)
        return cls(self, fh).originmd5
